...
export default function LocumSetupPage() {
    ...
    const searchCities = useCallback((q: string) => {
        ...
        let found = CITY_ROWS.filter((c) => c.name.toLowerCase().startsWith(lower)).slice(0, 8);
        if (found.length < 4) {
            const keys = new Set(found.map((c) => `${c.name}|${c.province}`));
            const extra = CITY_ROWS.filter((c) => !keys.has(`${c.name}|${c.province}`) &&
                c.name.toLowerCase().includes(lower)).slice(0, 8 - found.length);
            found = [...found, ...extra];
        }
    ...
    useEffect(() => {
        const handleOutside = (e: MouseEvent) => {
            ...
            if (cityDropRef.current?.contains(t) ||
                cityInputRef.current?.contains(t)) {
                return;
            }
            setCityDropOpen(false);
        };
      ...
      <div className="setup-modal" style={locumModalCardBase}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            gap: 24,
              ...
              {step === 1 && (<div style={{
                ...
                width: '100%',
                      ...
                      <textarea className="locum-setup-input" style={{
                ...inp,
                minHeight: 78,
                height: 78,
                resize: 'vertical',
            }} placeholder="Add Summary" value={form.professionalSummary} onChange={(e) => set('professionalSummary', e.target.value)}/>
                    </div>
                ...
                width: '100%',
                    ...
                    minWidth: 0,
                              ...
                              <select key={specSelectKey} aria-label="Pick specialization to add" style={{
                    ...selectSpecInp,
                    color: '#0B0F1F',
                }} value="" onChange={(e) => {
                    ...
                    appendSpecializationTag(v);
                                  ...
                                  {SPECIALIZATION_OPTIONS.filter((o) => !specializationTags.includes(o)).map((o) => (<option key={o} value={o}>
                                      {o}
                                    </option>))}
                                </select>
                                <span style={{
                    ...
                    boxSizing: 'border-box',
                        ...
                        {addingCustomSpec ? (<div ref={customSpecRowRef} style={{
                    display: 'flex',
                    flexDirection: 'row',
                    alignItems: 'stretch',
                    flexWrap: 'wrap',
                    ...
                    width: '100%',
                            ...
                            <input ref={customSpecRef} type="text" aria-label="Custom specialization" value={customSpec} onChange={(e) => setCustomSpec(e.target.value)} onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                        e.preventDefault();
                        confirmCustomSpec();
                    }
                ...
                }} placeholder="Type specialization…" style={{
                    flex: '1 1 160px',
                    minWidth: 0,
                    height: 44,
                    padding: '6px 8px',
                    ...
                    boxSizing: 'border-box',
                      ...
                      {specializationTags.length > 0 && (<div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                          {specializationTags.map((tag) => (<span key={tag} style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                        gap: 8,
                        padding: '8px 12px',
                        ...
                        letterSpacing: '0.02em',
                              ...
                              {tag}
                              <button type="button" onClick={() => updateSpecializationTags(specializationTags.filter((t) => t !== tag))} aria-label={`Remove ${tag}`} style={{
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        width: 16,
              ...
              {step === 2 && (<div style={{
                ...
                position: 'relative',
                            ...
                            <input id="locum-setup-city" ref={cityInputRef} className="locum-setup-input" style={{
                ...inp,
                borderColor: form.city && form.province
                    ? '#22C55E'
                    : undefined,
            ...
            }} onKeyDown={handleCityKeyDown} onFocus={() => {
                if (cityBlurTimer.current != null) {
                    clearTimeout(cityBlurTimer.current);
                }
                ...
                cityBlurTimer.current = setTimeout(() => setCityDropOpen(false), 160);
                    ...
                    overflowY: 'auto',
                        ...
                        textAlign: 'center',
                                    ...
                                    No city found
                                  </div>) : (cityResults.map((city, i) => (<div key={`${city.name}-${city.province}`} role="option" aria-selected={i === cityActiveIdx} onMouseDown={() => handleCitySelect(city)} style={{
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'space-between',
                        padding: '10px 14px',
                        cursor: 'pointer',
                        background: i === cityActiveIdx
                            ? 'rgba(15,42,122,0.05)'
                            : 'transparent',
                        ...
                        color: '#0B0F1F',
                                        ...
                                        {highlightCityMatch(city.name, form.city ?? '')}
                                      </span>
                                      <span style={{
                        fontSize: 11,
                        fontWeight: 700,
                ...
                gap: 8,
                    ...
                    form.province}
                              ...
                              </div>) : (<input id="locum-setup-province" className="locum-setup-input" style={{
                    ...inp,
              ...
              {step === 3 && (<div style={{
                ...
                color: '#9CA3AF',
                  ...
                  {err ? (<p style={{ fontSize: 14, color: '#dc2626', margin: 0 }}>
                      {err}
                    </p>) : null}
                </div>)}
            </div>
...
function UploadCloudIcon() {
    return (<svg width={24} height={24} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden style={{ flexShrink: 0 }}>
      <path stroke="#3A65DB" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" d="M12 16.5V9.75m0 0 3 3m-3-3-3 3M6.75 19.5a4.5 4.5 0 0 1-1.41-8.775 5.25 5.25 0 0 1 10.33-2.33 3 3 0 0 1 3.758 3.848A3.752 3.752 0 0 1 18 19.5H6.75Z"/>
    </svg>);
}
function NavButtons({ onBack, onNext, nextLabel = 'Next', disabled = false, nextGradient = false, }: {
    ...
    return (<div style={{ display: 'flex', gap: 12, width: '100%', alignItems: 'stretch' }}>
      {onBack ? (<button type="button" onClick={onBack} style={backStyle}>
          Back
        </button>) : null}
